...
enum L10n {
    ...
    private static let en: [String: String] = [
        ...
        "settings.engines.funasrNote": "Runs a local Python service tuned for Chinese, with 2-pass correction (streaming text, then an offline recheck that adds punctuation) and file import. Requires a one-time dependency install and model download on first use. Set the source language in the Languages section: FunASR only streams Chinese, so English is transcribed one sentence at a time.",
    ...
    private static let zh: [String: String] = [
        ...
        "settings.engines.funasrNote": "运行本地 Python 服务，支持二次复核（流式出字 + 离线复核修正加标点）和文件导入。首次使用会自动安装依赖并下载模型。请在“语言”一栏设置源语言：FunASR 只有中文流式模型，英文会按整句识别。",
