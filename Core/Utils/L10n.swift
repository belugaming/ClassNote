...
enum L10n {
    ...
    private static let en: [String: String] = [
        ...
        "settings.engines.funasrNote": "Runs a local Python service with streaming text and file import for both Chinese and English. Dependencies and models install automatically on first use. Set the source language in the Languages section: Chinese uses FunASR (streaming text, then an offline recheck that adds punctuation), English uses Nemotron (single pass, punctuation included).",
        ...
        "localASR.stage.punc": "Loading punctuation model…",
        "localASR.stage.warmup": "Warming up…",
        "settings.engines.notInstalled": "Not installed yet — about 1 GB will be downloaded",
        ...
        "settings.engines.nemotronNote": "Runs a local Python streaming service optimized for English. Requires a one-time dependency install on first use.",
    ...
    private static let zh: [String: String] = [
        ...
        "settings.engines.funasrNote": "运行本地 Python 服务，中英文都支持流式出字和文件导入。首次使用会自动安装依赖并下载模型。请在“语言”一栏设置源语言：中文用 FunASR（出字后再离线复核加标点），英文用 Nemotron（自带标点，单遍出结果）。",
        ...
        "localASR.stage.punc": "加载标点模型…",
        "localASR.stage.warmup": "预热模型…",
        "settings.engines.notInstalled": "尚未安装，需下载约 1 GB 模型",
        ...
        "settings.engines.nemotronNote": "运行本地 Python 流式服务，针对英文场景优化。首次使用需要安装一次依赖。",
